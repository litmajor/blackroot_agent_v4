...
# Fallback for black_vault if not present
try:
    from black_vault import BlackVault
    black_vault = BlackVault(password="default")
except Exception:
    black_vault = None
    ...
    for message in pubsub.listen():
        if message['type'] == 'message':
            try:
                ...
                swarm.inject_command(command, action)
...
def status():
    return {
        "peers": list(getattr(swarm, 'peers', {}).keys()),
...
async def recon_command(request: Request):
    ...
    # Use black_vault fallback if swarm.kernel.black_vault is not available
    vault = getattr(getattr(swarm, 'kernel', None), 'black_vault', black_vault)
    recon_module = ReconModule(vault, swarm, swarm.redis)
