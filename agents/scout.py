...
"""
from agent_core_anatomy import AgentID, AgentStatus, Event
...
class NodeProfile:
    ...
    def update_profile(self, data: Dict[str, Any]):
        ...
        for key, value in data.items():
            if key == 'node_id' and not isinstance(value, AgentID):
                setattr(self, key, AgentID(value))
    ...
    def from_dict(cls, data: Dict[str, Any]):
        ...
        if not isinstance(node_id, AgentID):
            node_id = AgentID(node_id)
        obj = cls(
            node_id=node_id,
...
class NetworkTopology:
    ...
    discovered_nodes: Dict[AgentID, NodeProfile] = field(default_factory=dict)
    connection_matrix: Dict[AgentID, List[AgentID]] = field(default_factory=dict)
    subnet_mapping: Dict[str, List[AgentID]] = field(default_factory=dict)
    risk_contours: Dict[AgentID, float] = field(default_factory=dict)
...
class TerrainMapper:
    ...
    def map_network_topology(self, seed_nodes: List[str]) -> NetworkTopology:
        ...
        for seed in seed_nodes:
            try:
                agent_id = AgentID(seed) if not isinstance(seed, AgentID) else seed
                path_tree = self._trace_network_paths(str(agent_id), self.resolution_depth)
                self._merge_topology_data(discovered_topology, path_tree)
            except Exception as e:
                logging.warning(f"Failed to map topology from seed {seed}: {str(e)}")
                continue
        discovered_topology.risk_contours = {AgentID(k) if not isinstance(k, AgentID) else k: v for k, v in self._calculate_risk_contours(discovered_topology).items()}
    ...
    def _merge_topology_data(self, topology: NetworkTopology, path_data: Dict[str, Any]) -> None:
        ...
        target = AgentID(path_data['target']) if not isinstance(path_data['target'], AgentID) else path_data['target']
        for hop_info in path_data['hops']:
            node_id = AgentID(hop_info['address']) if not isinstance(hop_info['address'], AgentID) else hop_info['address']
            if node_id not in topology.discovered_nodes:
                topology.discovered_nodes[node_id] = NodeProfile(
                    node_id=node_id,
                    ip_address=str(node_id)
                )
    ...
    def _evaluate_position_risk(self, node_id: AgentID, topology: NetworkTopology) -> float:
...
class NodeProfiler:
    ...
    def comprehensive_node_analysis(self, target_node: str) -> NodeProfile:
        ...
        agent_id = AgentID(target_node) if not isinstance(target_node, AgentID) else target_node
        node_profile = NodeProfile(
            node_id=agent_id,
            ip_address=str(agent_id)
        )
...
class ScoutAgent:
    ...
    def __init__(self, agent_id: str,
        ...
        self.agent_id = AgentID(f"{self.AGENT_ID_PREFIX}{agent_id}")
        self.initial_target_range = initial_target_range
        self.command_center_address = command_center_address
        self.known_nodes: Dict[AgentID, NodeProfile] = {}  # {AgentID: NodeProfile}
        ...
        self.operational_status: AgentStatus = AgentStatus.DORMANT
        ...
        self.black_vault = None  # Will be injected by kernel for secure storage

    def _setup_logger(self, level: int) -> logging.Logger:
        """Configures the agent's logger."""
        logger = logging.getLogger(str(self.agent_id))
    ...
    async def start(self):
        ...
        self.logger.info(f"{str(self.agent_id)} starting operations.")
        self.operational_status = AgentStatus.ACTIVE
        ...
        for target in self.initial_target_range:
            agent_id = AgentID(target) if not isinstance(target, AgentID) else target
            await self.scan_queue.put({'target': agent_id, 'scan_depth': self.current_tactical_mode_config['scan_depth']})

        await asyncio.gather(*self.running_tasks)

    async def stop(self):
        """Signals the agent to stop operations gracefully."""
        self.logger.info(f"{str(self.agent_id)} stopping operations.")
        self.operational_status = AgentStatus.SUSPENDED
    ...
    async def _perform_scan(self, target_ip: str, depth: int) -> Dict[str, Any]:
        ...
        node_id = AgentID(target_ip) if not isinstance(target_ip, AgentID) else target_ip
        if node_id not in self.known_nodes:
            self.known_nodes[node_id] = NodeProfile(node_id=node_id, ip_address=str(node_id))
        node_profile = self.known_nodes[node_id]
    ...
    async def _analysis_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self.known_nodes[node_profile.node_id] = node_profile # Update the global known_nodes map
